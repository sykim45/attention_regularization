...
class PyramidNet_noise_reconstruct(nn.Module):

    def __init__(self, depth, alpha, num_classes, drop_rate, is_imagenet=False, bottleneck=False):
        super(PyramidNet_noise_reconstruct, self).__init__()
        ...
        if self.is_imagenet is False:
            ...
            self.dropout = RmNoiseMix(self.linear.weight, self.linear.bias, self.drop_rate,
                                        device=torch.device('cuda'), mode='mean')
