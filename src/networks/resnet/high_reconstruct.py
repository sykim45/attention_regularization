...
from networks.subnet.dropout import DetectionMix, RmNoiseMix
...
class ResNet_high_reconstruct(nn.Module):
    def __init__(self, depth, num_classes, dropout_rate, is_imagenet):
        super(ResNet_high_reconstruct, self).__init__()
        self.in_planes = 64
        ...
        self.dropout = DetectionMix(self.linear.weight, self.linear.bias, self.drop_rate, device=torch.device('cuda'))
        #self.dropout = RmNoiseMix(self.linear.weight, self.linear.bias, self.drop_rate, device=torch.device('cuda'))
    ...
    def forward(self, x, rand_idx=None):
        if rand_idx is not None:
            ...
            out, ratio = self.dropout(gap, gap_f, self.linear(gap), self.linear(gap_f))
            #out = self.linear(out)
            return out, ratio
        else:
            out = F.relu(self.bn1(self.conv1(x)))
