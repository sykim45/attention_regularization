...
from networks.subnet.block_dropout import BottleneckDropout
from networks.subnet.dropout import CAMDropout
...
class ResNet_att(nn.Module):
    def __init__(self, depth, num_classes, dropout_rate, is_immagenet):
        ...
        self.dropout = CAMDropout(self.linear.weight, self.linear.bias, self.drop_rate, device=torch.device('cuda'))
    ...
    def forward(self, x):
        ...
        out = self.dropout(gap, self.linear(gap))
        return out