...
from networks.subnet.dropout import StochasticReconstruct
...
class ResNet_random_reconstruct(nn.Module):
    def __init__(self, depth, num_classes, dropout_rate, is_imagenet):
        super(ResNet_random_reconstruct, self).__init__()
        self.in_planes = 64
        ...
        self.dropout = StochasticReconstruct(self.linear.weight, self.linear.bias, self.drop_rate, device=torch.device('cuda'))
